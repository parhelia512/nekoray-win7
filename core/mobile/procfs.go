...
// Copied from the fork's experimental/libbox/internal/procfs, which the module boundary keeps private.
var (
	procNetIndexOfLocal = -1
	procNetIndexOfUid   = -1
...
func resolveSocketByProcSearch(network string, source, _ netip.AddrPort) int32 {
	if procNetIndexOfLocal < 0 || procNetIndexOfUid < 0 {
		return -1
	}
	...
	for {
		...
		if len(fields) <= procNetIndexOfLocal || len(fields) <= procNetIndexOfUid {
			continue
		}

		if strings.EqualFold(local, fields[procNetIndexOfLocal]) {
			uid, err := strconv.Atoi(fields[procNetIndexOfUid])
			if err != nil {
				return -1
			}
...
func init() {
	...
	for idx, col := range columns {
		...
		case "tm->when":
			tmWhen = true
		case "local_address":
			procNetIndexOfLocal = idx + offset
		case "uid":
			procNetIndexOfUid = idx + offset
		}
	}
}
