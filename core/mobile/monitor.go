...
// libbox's FixAndroidStack path (golang/go#68760) made unconditional: the call arrives on a Java
// thread, so the update runs on a fresh goroutine and the caller waits for it.
